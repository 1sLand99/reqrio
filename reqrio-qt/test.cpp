//
// Created by XLX on 2026/1/1.
//

#include <iostream>
#include <ostream>
#include <QJsonArray>
#include <QJsonDocument>

#include "Algorithm.h"
#include "CipherSuite.h"
#include "CompressionMethod.h"
#include "EcPointFormat.h"
#include "ExtensionType.h"
#include "Group.h"
#include "Session.h"
#include "Version.h"
#include "WebSocket.h"


void get() {
    const auto *session = new Session(ALPN::HTTP20);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
    qDebug() << resp.text().mid(0, 100);
    for (Cookie cookie: resp.cookies()) {
        qDebug() << cookie.getName() << "=" << cookie.getValue();
    }

    const auto url = new Url("https://153.3.238.127", "www.baidu.com");
    url->addParam("a", "3");
    url->addParam("b", R"({"fgdg":"dg"})");
    const Response resp1 = session->get(url, new Body());
    qDebug() << resp.statusCode();
    delete session;
}

void post_form() {
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    QMap<QString, QString> forms;
    forms.insert("a", "3");
    forms.insert("b", R"({"fgdg":"dg"})");
    const Response resp = session->post("https://www.baidu.com", new Body(forms));
    qDebug() << resp.statusCode();

    const Response resp1 = session->post("https://www.baidu.com", forms);
    qDebug() << resp1.statusCode();
    delete session;
}

void post_json() {
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    QJsonObject json;
    json.insert("a", "3");
    QJsonObject obj;
    obj.insert("fgdg", "dg");
    json.insert("b", obj);
    const Response resp = session->post("https://www.baidu.com", QJsonDocument(json));
    qDebug() << resp.statusCode();
    delete session;
}

QString TOKEN = "";

void client_hello_finger() {
    printf("================>[TLS WITH CL]<==================\n");
    const auto client_hello = QByteArray::fromHex(
        "");
    const auto fingerprint = Fingerprint::fromClientHello(client_hello, TOKEN);
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->setFingerprint(fingerprint);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    session->addHeader("sec-ch-ua", R"("Microsoft Edge";v="131", "Chromium";v="131", "Not_A Brand";v="24")");
    session->addHeader("sec-ch-ua_mobile", "?0");
    session->addHeader("sec-ch-ua_platform", "\"Linux\"");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
}

void ja3() {
    printf("================>[JA3 TLS]<==================\n");
    const auto fingerprint = Fingerprint::fromJa3(
        "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513-21,29-23-24,0",
        TOKEN);
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->setFingerprint(fingerprint);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
}

void ja4() {
    printf("================>[JA4 TLS]<==================\n");
    const auto fingerprint = Fingerprint::fromJa4(
        "t13d1516h2_002f,0035,009c,009d,1301,1302,1303,c013,c014,c02b,c02c,c02f,c030,cca8,cca9_0005,000a,000b,000d,0012,0017,001b,0023,002b,002d,0033,44cd,fe0d,ff01_0403,0804,0401,0503,0805,0501,0806,0601",
        TOKEN);
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->setFingerprint(fingerprint);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
}

void rand_tls() {
    printf("================>[RANDOM TLS]<==================\n");
    const auto fingerprint = Fingerprint::random(TOKEN);
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->setFingerprint(fingerprint);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
}

struct Finger {
    QList<int> suites;
    QList<int> compress_methods;
    QJsonObject extensions;
    QJsonObject settings;
    int window_size;
    int weight;
    bool priority;
};

void custom_finger() {
    printf("================>[CUSTOM TLS]<==================\n");
    Finger finger = {
        {
            TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
            TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,
            TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256,
            TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA,
            TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA,
            TLS_AES_128_GCM_SHA256,
            TLS_AES_256_GCM_SHA384,
            TLS_CHACHA20_POLY1305_SHA256
        },
        {NUL},
        {
            {QString::number(StatusRequest), QJsonValue::Null},
            {
                QString::number(SupportedGroup), QJsonArray{
                    X25519,
                    Secp256r1,
                    Secp384r1,
                    Secp521r1,
                },
            },
            {QString::number(EcPointFormats), QJsonArray{UNCOMPRESSED}},
            {
                QString::number(SignatureAlgorithms), QJsonArray{
                    RSA_PKCS1_SHA1,
                    RSA_PKCS1_SHA256,
                    RSA_PKCS1_SHA384,
                    RSA_PKCS1_SHA512,
                    ECDSA_SECP256R1_SHA256,
                    ECDSA_SECP384R1_SHA384,
                    ECDSA_SECP521R1_SHA512,
                    RSA_PSS_PSS_SHA256,
                    RSA_PSS_PSS_SHA384,
                    RSA_PSS_PSS_SHA512,
                    RSA_PSS_RSAE_SHA256,
                    RSA_PSS_RSAE_SHA384,
                    RSA_PSS_RSAE_SHA512,
                }
            },
            {QString::number(SignedCertificateTimestamp), QJsonValue::Null},
            {QString::number(ExtendMasterSecret), QJsonValue::Null},
            {QString::number(CompressionCertificate), QJsonArray{NUL}},
            {QString::number(SessionTicket), QJsonValue::Null},
            {
                QString::number(SupportedVersions), QJsonArray{
                    0xeaea,
                    TLS_1_3,
                    TLS_1_2,
                }
            },
            {QString::number(PskKeyExchangeMode), QJsonValue::Null},
            {
                QString::number(KeyShare), QJsonArray{
                    X25519,
                    Secp256r1,
                }
            },
            {
                QString::number(ApplicationSetting), QJsonArray{
                    HTTP20,
                    HTTP11
                }
            },
            {QString::number(ServerName), QJsonValue::Null},
            {
                QString::number(ApplicationLayerProtocolNegotiation), QJsonArray{
                    HTTP20,
                    HTTP11
                }
            },
            {QString::number(Padding), 12},
            {QString::number(0xdada), QJsonArray{0}}
        },
        {
            {"HeaderTableSize", 65536},
            {"EnablePush", 0},
            {"InitialWindowSize", 6291456},
            {"MaxHeaderListSize", 242144}
        },
        2147418112,
        147,
        true
    };
    // QJsonDocument document(finger);
    qDebug() << finger.extensions;


    const auto fingerprint = Fingerprint::random(TOKEN);
    const auto *session = new Session(ALPN::HTTP20);
    session->setRedirect(false);
    session->setFingerprint(fingerprint);
    session->addHeader("User-Agent",
                       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36 Edg/143.0.0.0");
    const Response resp = session->get("https://www.baidu.com");
    qDebug() << resp.statusCode();
}


// void callback(const char *data, uint32_t len) {
//     QByteArray bytes = QByteArray::fromRawData(data, len);
//     qDebug() << bytes.length();
// }

int main(int argc, char *argv[]) {
    // get();
    // post_form();
    // post_json();
    // client_hello_finger();
    // ja3();
    // ja4();
    // rand_tls();
    custom_finger();


    // Session session(HTTP20);
    // Response resp = session.get(new Url("https://m.so.com"), new Body());
    // qDebug() << resp.statusCode();
    // qDebug() << resp.text() << resp.text().length();

    // try {
    //     WebSocket webSocket("wss://alive.github.com");
    //     webSocket.open();
    // } catch (std::exception &e) {
    //     qDebug() << e.what();
    // }

    // QJsonObject obj = webSocket.read();
}
