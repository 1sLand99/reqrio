...
async fn main() {
    #[cfg(feature = "log")]
    test_log();
    let mask = [34, 47, 123, 231];
    let mut v = [1, 2, 3, 4];
    let s = u32::from_be_bytes(v);
    println!("{}", s);
    v.iter_mut().enumerate().for_each(|(i, x)| *x ^= mask[i]);
    let s = u32::from_be_bytes(v);
    let vvv = u32::from_be_bytes(mask);
    println!("{}", s ^ vvv);
    return;
    ...
    let resp = req.get(url, body.ty(ContentType::Null)).await.unwrap();
