...
async fn main() {
    ...
    let mut req = AcReq::new().with_timeout(Timeout::new_same(3000,1)).with_key_log("2.log");
    ...
    let res=req.get("https://music.163.com/program?id=901456263",None).await.unwrap();
    ...
    let mut req = AcReq::new()
        ...
        // .with_proxy(proxy)
        .with_verify(false)
        // .with_proxy(Proxy::try_from("http://60.215.152.144:20262").unwrap())
        // .with_mtls(certs, key)
