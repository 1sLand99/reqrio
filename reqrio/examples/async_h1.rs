use reqrio::*;
use reqtls::coder::ZSTDEncode;

#[tokio::main]
async fn main() {
    let buf = rand::random::<[u8; 16 * 1024]>();
    let mut encode = ZSTDEncode::new().unwrap();
    let mut dd = vec![];
    for chunk in buf.chunks(4096) {
        let e = encode.encode(chunk).unwrap();
        println!("chuck: {}", e.len());
        dd.extend_from_slice(e);
    }
    dd.extend_from_slice(encode.finalize().unwrap());
    println!("{}", dd.len());


    // let d = reqrio::coder::zstd_encode(buf).unwrap();
    // println!("{}", d.len());
    let mut decode = reqtls::coder::ZSTDDecode::new().unwrap();
    let mut res = vec![];
    for chunk in dd.chunks(1024) {
        res.extend_from_slice(decode.decode(chunk).unwrap())
    }
    println!("{}", res.len());


    // println!("{:?}", decode.decode(&d[8192..]).unwrap().len());


    // let f = reqrio::coder::zstd_decode(&d).unwrap();
    // println!("{:?}", f);
    // let x = reqtls::coder::zstd_decompress(d).unwrap();
    // println!("{:?}", x);
