use reqrio::{json, Buffer, Fingerprint, ReqExt, ScReq, Timeout, ALPN};
use reqtls::WriteExt;

fn main() {
    let mut buffer = Buffer::with_capacity(1024);
    buffer.write_u32(1212121232).unwrap();
    buffer.write_u24(1212121232).unwrap();
    buffer.write_u8(32).unwrap();
    buffer.write_u16(12334).unwrap();
    println!("{:?}", buffer.filled());
    println!("{:?}", 1212121232u32.to_be_bytes());
    println!("{:?}", 12334u16.to_be_bytes());
