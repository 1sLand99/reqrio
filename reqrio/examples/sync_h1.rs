use reqrio::{json, Fingerprint, Proxy, ReqExt, ScReq, Time, Timeout, ALPN};
...
fn main() {
    ...
    let mut req = ScReq::new()
        ...
        .with_verify(false)
        .with_key_log("2.log")
        .with_proxy(Proxy::try_from("http://218.29.54.12:11808").unwrap())
        // .with_proxy(Proxy::new_socks5("184.181.217.194",4145))
        // .with_proxy(Proxy::new_http_plain("127.0.0.1",10279))
        ;
    let res = req.get("https://baidu.com/", None).unwrap();
