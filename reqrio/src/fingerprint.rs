...
pub struct Fingerprint {
    pub tls: TlsFinger,
    pub h2_setting: H2Frame<'static>,
    pub h2_window_update: H2Frame<'static>,
    pub legal_subscript: i32,
}

impl Fingerprint {
    pub(crate) fn h2_setting(&self) -> &H2Frame<'static> {
        &self.h2_setting
    }

    pub(crate) fn h2_window_update(&self) -> &H2Frame<'static> {
...
impl Fingerprint {
    pub fn set_h2_setting(&mut self, setting: H2Frame<'static>) -> Result<(), BufferError> {
        self.h2_setting = setting;
        Ok(())
    }

    pub fn set_h2_window_update(&mut self, update: H2Frame<'static>) -> Result<(), BufferError> {
        self.h2_window_update = update;
        Ok(())
    }
    ...
    pub fn from_hex_all(hex_str: impl AsRef<str>, token: impl AsRef<str>) -> HlsResult<Fingerprint> {
        ...
        let mut client_key_exchange = client_hello.split_off(len);
        let len = u16::from_be_bytes([client_key_exchange[3], client_key_exchange[4]]) as usize + 5;
        let change_cipher_spec = client_key_exchange.split_off(len);
        Ok(Fingerprint {
            tls: TlsFinger::TLS12HandShake {
                client_hello: Bytes::new(client_hello),
                client_key_exchange: Bytes::new(client_key_exchange),
                change_cipher_spec: Bytes::new(change_cipher_spec),
            },
            legal_subscript: Buffer::with_capacity(0).check_subscription(token)?,
            ..Default::default()
        })
