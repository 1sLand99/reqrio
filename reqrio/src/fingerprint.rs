...
impl Fingerprint {
    pub fn new() -> Fingerprint {
        Fingerprint {
            ...
            h2_setting: Buffer::with_capacity(0),
            h2_window_update: Buffer::with_capacity(0),
            legal_subscript: -2,
        }
    }

    pub(crate) fn h2_setting(&self) -> &[u8] {
        self.h2_setting.filled()
    }

    pub(crate) fn h2_window_update(&self) -> &[u8] {
        self.h2_window_update.filled()
    }
...
impl Debug for Fingerprint {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Fingerprint {{")?;
        write!(f, "client_hello: {}", hex::encode(&self.client_hello))?;
        write!(f, "client_key_exchange: {}", hex::encode(&self.client_key_exchange))?;
        write!(f, "change_cipher_spec: {}", hex::encode(&self.change_cipher_spec))?;
        write!(f, "h2_setting: {}", hex::encode(self.h2_setting.filled()))?;
        write!(f, "h2_window_update: {}", hex::encode(self.h2_window_update.filled()))?;
        write!(f, "legal_subscript: {}", self.legal_subscript)?;
        write!(f, "}}")
    }
}
...
impl Default for Fingerprint {
    fn default() -> Fingerprint {
        Fingerprint {
            ...
            h2_setting: Buffer::new_bytes(vec![0, 0, 24, 4, 0, 0, 0, 0, 0, 0, 1, 0, 0, 255, 255, 0, 2, 0, 0, 0, 0, 0, 4, 0, 96, 0, 0, 0, 6, 0, 3, 177, 224]),
            h2_window_update: Buffer::new_bytes(vec![0, 0, 4, 8, 0, 0, 0, 0, 0, 127, 255, 0, 0]),
            legal_subscript: -2,
        }
    }
}