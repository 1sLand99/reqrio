...
impl QPackDecode {
    ...
    pub fn decode_into(&mut self, buf: &[u8], header: &mut Header, typ: QPackType, sid: &u64) -> HlsResult<usize> {
...
mod tests {
    ...
    fn test_qpack_decode4() {
        ...
        decoder.decode_into(&data, resp.header_mut(), QPackType::Stream, &0).unwrap();
        ...
        let mut resp = Response::new();
        decoder.decode_into(&data, resp.header_mut(), QPackType::Stream, &8).unwrap();
        ...
        let mut resp = Response::new();
        decoder.decode_into(&data, resp.header_mut(), QPackType::Stream, &12).unwrap();
        assert_eq!(resp.header().status(), &HttpStatus::OK);
    }
}