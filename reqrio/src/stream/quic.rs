...
use crate::HlsError;
use reqtls::quic::*;
use reqtls::{rand, Buf, Buffer, Config, PacketType, StreamHandle, StreamParam};
...
impl QUICStreamS {
    pub fn connect(socket: UdpSocket, remote_addr: SocketAddr, mut config: Config<'_>) -> HlsResult<QUICStreamS> {
    ...
    fn read_frames(&mut self) -> HlsResult<Vec<QUICFrame<'_>>> {
...
mod tests {
    ...
    use reqtls::{ClientConfig, Config, RsaKey, TlsFinger, ALPN};
