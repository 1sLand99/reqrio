...
mod tests {
    ...
    #[test]
    fn test_p256() {
        let mut p256 = EcCurve::new_p256().unwrap();
        let pub_key = p256.pub_key();
        ...
        let mut p384 = EcCurve::new_p384().unwrap();
        let pub_key = p384.pub_key();
        println!("{} {:?}", pub_key.len(), pub_key);
        ...
        let mut p521 = EcCurve::new_p521().unwrap();
        let pub_key = p521.pub_key();
        println!("{} {:?}", pub_key.len(), pub_key);
