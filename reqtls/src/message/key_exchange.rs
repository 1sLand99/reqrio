...
impl<'a> ServerKeyExchange<'a> {
    pub fn from_reader(ht: HandshakeType, mut reader: Reader<'a>) -> RlsResult<ServerKeyExchange<'a>> {
        reader.read_u32_24()?;
        Ok(ServerKeyExchange {
            handshake_type: ht,
            hellman_param: ServerHellmanParam::from_reader(&mut reader)?,
        })
    }
...
impl<'a> ClientHellmanParam<'a> {
    ...
    pub fn from_reader(reader: &mut Reader<'a>, suite: Option<&CipherSuite>) -> RlsResult<ClientHellmanParam<'a>> {
        ...
        res.pub_key = Buf::Ref(reader.read_slice(key_size as usize)?);
        Ok(res)
    }
...
impl<'a> ClientKeyExchange<'a> {
    pub fn from_reader(ht: HandshakeType, mut reader: Reader<'a>, suite: Option<&CipherSuite>) -> RlsResult<ClientKeyExchange<'a>> {
        reader.read_u32_24()?;
        Ok(ClientKeyExchange {
            handshake_type: ht,
            hellman_param: ClientHellmanParam::from_reader(&mut reader, suite)?,
        })
    }
